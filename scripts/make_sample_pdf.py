...
OUT = PDFS_DIR/"sales_report.pdf"
W,H = 1240,1754
def _font(size:int):
    try:
        return ImageFont.truetype("some/font/path.ttf",size)
    except OSError:
        return ImageFont.load_default()
def make_chart_page() -> Image.Image:
    img = Image.new("RGB", (W, H), "white")
    d = ImageDraw.Draw(img)
    d.text((90, 70), "Acme Revenue - Quarterly (Rs Crore)", font=_font(46), fill="black")
    ...
    for i, (label, value) in enumerate(quarters):
        ...
        d.text((x0 + 20, base_y - bar_h - 40), str(value), font=_font(40), fill="black")
        d.text((x0 + 20, base_y + 20), label, font=_font(40), fill="black")
    return img
